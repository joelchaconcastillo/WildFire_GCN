...
class CNN(nn.Module):
   def __init__(self, dim_out):
      ...
      self.features = nn.Sequential(
          nn.Conv2d(1, int(dim_out/2), kernel_size=3, stride=2), #channel of ZPI is 1
    ...
    def sieve(self, alpha : float = 1.0):
      ...
      for (dynamic, static, clc, prefix) in self.data:
          ...
          for time in range(windowSize):
              ...
              x = torch.tensor(X.reshape(feat, -1))
              ####convert to tensor..
              ...
              graph_cnn = cnn(adjMatrix) #B, hidden_dim/2, hidden_dim/2
...
#              L2[L2 > alpha] = 0.0 ##is this necessary?
              graphWindow[time] = graph_cnn.numpy()
          np.savez_compressed(prefix+"_graph_cnn_"+hidden_dim, graph=graphWindow)
          print(prefix)
