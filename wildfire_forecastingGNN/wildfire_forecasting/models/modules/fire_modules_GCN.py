...
class GCN(nn.Module):
   def __init__(self, hparams: dict):
      ...
      if hparams['clc'] == 'vec':
          input_dim += 10
      self.num_nodes = int(hparams['patch_width'])*int(hparams['patch_height'])
      self.hidden_dim = hparams['rnn_units']
      self.input_dim = hparams['input_dim']
      self.output_dim = hparams['output_dim']
      self.num_layers = hparams['num_layers']
      self.embed_dim = hparams['embed_dim']
      self.window_len = hparams['window_len']
      self.link_len = hparams['link_len']
